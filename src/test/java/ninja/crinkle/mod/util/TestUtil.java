...
public class TestUtil {
    ...
    private static <T> void mockField(Class<T> type, String name, T target, Object value) {
        Field field = ReflectionUtils.findFields(type, f -> f.getName().equals(name),
    ...
    public static Theme spyTheme() {
        ...
        theme.addStyle(spyWidgetTheme(theme));
        return theme;
    }
    ...
    public static Style spyWidgetTheme(Theme theme) {
        Style.Builder b = Style.builder("panel")
                .theme(theme)
                .addVariant(Style.Variant.active, spyAppearance(theme));
        return spy(b.build());
    }
